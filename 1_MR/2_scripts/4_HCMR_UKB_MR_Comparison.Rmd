...
ukb_mr <- read_tsv(str_c('1_MR/3_output/2_MR/HCM_to_ukb_',pp,'/HCM_to_ukb_',pp,'_instrument_details.tsv')) %>% dplyr::rename('beta.outcome.ukb'='beta.outcome','se.outcome.ukb'='se.outcome') %>% select(SNP, beta.outcome.ukb, se.outcome.ukb)

hcmr_mr <- read_tsv(str_c('1_MR/3_output/2_MR/HCM_HCMRless_to_hcmr_',pp,'/HCM_HCMRless_to_hcmr_',pp,'_instrument_details.tsv')) %>% dplyr::rename('beta.outcome.hcmr'='beta.outcome','se.outcome.hcmr'='se.outcome') %>% select(SNP, beta.outcome.hcmr, se.outcome.hcmr)
```
...
print(comp_plot)
ggsave(str_c('1_MR/3_output/3_summary_plots/', 'hcm_to_ntprobnp_ukb_hcmr_betacomp.png'), dpi=600)
```
